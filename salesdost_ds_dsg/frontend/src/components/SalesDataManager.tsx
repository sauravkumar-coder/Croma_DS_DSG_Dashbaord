...
export default function SalesDataManager({ onClose, onDataChanged }: SalesDataManagerProps) {
  ...
  const { brandFrom, brandTo } = retailerCfg
  ...
  const fetchMeta = useCallback(async () => {
    try {
      const { data } = await getRetailerSalesMeta(retailer)
      setMeta(data)
    } catch {
  ...
  const handleFileSelected = useCallback(async (file: File, force = false) => {
    ...
    try {
      const { data } = await uploadRetailerSales(
        retailer, file,
        pct => setPanelState({ kind: 'uploading', progress: pct }),
        force,
      )
      ...
      const msg =
        ...
        'Upload failed. Check the file format and sheet name (RAW).'
      setPanelState({ kind: 'error', message: msg })
    }
  }, [fetchMeta, onDataChanged, retailer])

  const handleClear = useCallback(async () => {
    setPanelState({ kind: 'clearing' })
    try {
      await deleteRetailerSales(retailer)
      setMeta({ loaded: false })
      onDataChanged()
  ...
  }, [retailer, fetchMeta, onDataChanged])
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900">{retailerCfg.label} Data Management</p>
              <p className="text-[10px] text-gray-400 leading-tight">Upload · Replace · Clear</p>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-blue-700">
                    {meta.loaded ? 'Upload New / Replace File' : `Upload ${retailerCfg.label} File`}
                  </p>
                  <p className="text-[11px] text-blue-500 mt-0.5">
                    {meta.loaded
                      ? 'Replaces current dataset and recalculates all KPIs'
                      : '.xlsx / .xls · RAW sheet · validates and recalculates'}
                  </p>
                </div>
              </button>
