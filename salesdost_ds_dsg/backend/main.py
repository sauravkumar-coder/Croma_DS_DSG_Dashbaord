...
async def get_dashboard_data(retailer: str = ""):
    ...
    # Determine target month dynamically from StoreTarget for brand_007
    target_month_num = 6
    target_year = 2026
    target_doc = await db["StoreTarget"].find_one({"brandId": "brand_007"})
    if target_doc:
...
async def get_store_detail(store_id: str, retailer: str = ""):
    ...
    sb_doc = await db["StoreBrand"].find_one({"storeBrandId": store_id, "brandId": "brand_007"})
